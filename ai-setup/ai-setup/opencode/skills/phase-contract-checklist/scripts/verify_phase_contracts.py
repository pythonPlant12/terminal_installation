...
def require_non_empty_list(phase: dict, field: str, errors: list[str], label: str) -> None:
...
def main() -> int:
    ...
    parser.add_argument("--contracts", default="phase-contracts.json", help="Path to phase contracts JSON file")
    ...
    for idx, phase in enumerate(phases, start=1):
        ...
        if not isinstance(must_run_after, list) or not all(isinstance(item, str) and ID_RE.match(item) for item in must_run_after):
            errors.append(f"{phase_ref}: 'must_run_after' must be a list of two-digit ids")
        behavior_markers = phase.get("behavior_markers", [])
        if not isinstance(behavior_markers, list) or not all(isinstance(item, str) and item.strip() for item in behavior_markers):
            errors.append(f"{phase_ref}: 'behavior_markers' must be a list of non-empty strings")
    ...
    for phase in phases:
        ...
        label = phase.get("label")
        script = phase.get("script")
        if not (isinstance(phase_id, str) and isinstance(label, str) and isinstance(script, str)):
            continue
        ...
        if ann_id is None:
            ...
            errors.append(f"{phase_id}: PHASE_ID annotation '{ann_id}' does not match contract id")
        if ann_label is None:
            errors.append(f"{phase_id}: missing '# PHASE_LABEL:' annotation in '{script}'")
        elif ann_label != label:
            errors.append(f"{phase_id}: PHASE_LABEL annotation '{ann_label}' does not match contract label")

        for marker in phase.get("behavior_markers", []):
            if marker not in content:
                errors.append(f"{phase_id}: behavior marker '{marker}' not found in '{script}'")

        for dep in phase.get("must_run_after", []):
            if dep not in phase_ids:
                errors.append(f"{phase_id}: dependency '{dep}' in must_run_after does not exist")
                continue
            if dep >= phase_id:
                errors.append(f"{phase_id}: dependency '{dep}' must be numerically lower than phase id")
